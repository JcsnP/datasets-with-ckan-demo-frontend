...
import { Button, Modal, Form, Card, Dropdown, DropdownButton } from 'react-bootstrap';
	...
	if(!success) {
		return(
	      ...
	      >
	        ...
	        <Modal.Body>
	        	<Form>
		          <Form.Group className='mb-3 fw-bold'>
		          	...
		          	<Form.Text className="text-muted d-flex gap-1 align-items-center">
		          		...
		          		<div className='border w-100 border rounded p-1'>
		          			{link.length === 0 ? '‎' : link.toLowerCase().replaceAll(',', '').replaceAll(' ', '-')}
		          		</div>
		          	</Form.Text>
		          </Form.Group>
		          ...
		          <Form.Group className='mb-2 fw-bold'>
		          	<Form.Label>
		          		<FontAwesomeIcon icon={faEye} size="xs" style={{color: "#000000",}} className='me-1' />
		          		Privacy
		          	</Form.Label>
		          	<Form.Select aria-label="Select privacy">
	        ...
	        <Modal.Footer>
	          ...
	          <Button variant="primary" onClick={() => {createDatasets()}}>Create</Button>
	        </Modal.Footer>
	      </Modal>
	    </>
		);
		...
		return(
	      ...
	      <Modal
	        ...
	        keyboard={false}
	      >
	        <Modal.Body className='d-flex align-items-center justify-content-center w-100' style={{height: '15em'}}>
	        	<div className='text-center' style={{fontSize: '4em', color: '#4BB543'}}>
